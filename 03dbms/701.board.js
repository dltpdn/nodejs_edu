...
app.get('/detail/:id', function(req, res){
	fs.readFile(__dirname +'/public/detail.html', 'utf8', function(err, data){
		...
		conn.query('SELECT * FROM board WHERE id=?',[req.params.id], function(err, rs, field){
			console.log(rs);
...
app.post('/save/:id', function(req, res, next){
	...
	if(mode === 'edit'){
		conn.query('UPDATE board SET title=?, content=?, writer=? WHERE id=?', [title, content, writer, id], function(err, rs, field){
...
app.get('/delete/:id', function(req, res, next){
	var id = req.params.id;
	conn.query('DELETE FROM board WHERE id=?', [id], function(err, rs, field){
		if(!err){
			res.redirect('/');
		}else{
			next(err, req, res);
